...
%using svm regression model(libsvm) generated from
%svm_regression_generator to calculate each user's potential interesting
%films.
function svm_regression_test()
%108,133,228,232,336,338,545,613,696,777
  ...
  output_path='../out/';
  fid=fopen([output_path,'t2.txt'],'w+');
  ...
  load([dataset_path,'svm_regression_model.mat']);                          %load svm regression model.
...
%               [predict,accuracy,decision_value] = svmpredict(3,feat_tmp,model); 
              ...
              FilmScore_Occupation(j,:)];                                                   %film-occupation socre.
              [predict,accuracy,decision_value] = svmpredict(3,feat_tmp,model); 
              predict_score(j,2)=predict;
      end
      
    predict_score=sortrows(predict_score,-2);                    %descent sort by column 2, the predict score.
      for j=1:5
         disp(['Film Id: ',num2str(predict_score(j,1)),' predicted score:', num2str(predict_score(j,2))]);
         fprintf(fid,['Film Id: ',num2str(predict_score(j,1)),' predicted score:', num2str(predict_score(j,2)),'\n']);
      end
  end
